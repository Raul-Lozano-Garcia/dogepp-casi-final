...
<body>
    ...
    <section id="panelParques" class="container panelAdmin">
    ...
    <?php
        ...
        //SACO LOS USUARIOS
        $consulta_mensaje="SELECT id,nombre,descripcion,imagen,activo FROM parque";
        ...
        if ($filas_devueltas>0) {
            echo "<div class='row'>
                <table>
                <tr><th>Nombre</th><th>Descripcion</th><th>Foto</th><th></th><th></th></tr>";
            while ($fila_mensaje=$datos_mensaje->fetch_array(MYSQLI_ASSOC)) {
                echo "<tr class='fila_a_buscar'>
                        ...
                        if($fila_mensaje["activo"]==='0'){
                            echo "<td><img src='../../assets/imagenes/city_dog/parques/$fila_mensaje[imagen]' class='img-fluid desactivada'></td>";
                        }else{
                            echo "<td><img src='../../assets/imagenes/city_dog/parques/$fila_mensaje[imagen]' class='img-fluid'></td>";
                        }
                        
                        echo "<td>
                            <form method='POST' action='ver_perfil_parque.php'>
            ...
            echo "<h1 class='fw-bold text-center'>No hay parques aún</h1>";
        }
    ...
    <?php
        if(isset(($_POST["borrar"]))){
            ...
            $consulta_borrar="UPDATE parque SET activo='0' WHERE id=?";
            ...
            $consulta_borrar="UPDATE parque SET activo='1' WHERE id=?";
            $resultado_borrar=$conexion->prepare($consulta_borrar);
            $resultado_borrar->bind_param("i", $id);
            $resultado_borrar->execute();
            $resultado_borrar->close();
