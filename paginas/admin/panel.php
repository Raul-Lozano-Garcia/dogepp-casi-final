...
<body>
    ...
    <!-- BUSCADOR -->
    <span class="d-block text-end">Buscar por nombre o nick: <input type="search" id="search"></span>
        
    <?php

        //SACO LOS USUARIOS
        $consulta_mensaje="SELECT dni,nombre,nick,tipo,imagen,activo FROM usuario WHERE dni<>'$_SESSION[dni]'";
        ...
        if ($filas_devueltas>0) {
            echo "<div class='row'>
                <table>
                <tr><th>DNI</th><th>Nombre (Nick)</th><th>Tipo</th><th>Foto</th><th></th><th></th></tr>";
            while ($fila_mensaje=$datos_mensaje->fetch_array(MYSQLI_ASSOC)) {
                echo "<tr class='fila_a_buscar'>
                        <td>$fila_mensaje[dni]</td>
                        <td><h2>$fila_mensaje[nombre] ($fila_mensaje[nick])</h2></td>";

                        if($fila_mensaje["tipo"]==='a'){
                            echo "<td>Adiestrador</td>";;
                        }else{
                            echo "<td>Básico</td>";;
                        }

                        if($fila_mensaje["activo"]==='0'){
                            echo "<td><img src='../../assets/imagenes/usuarios/perfil/$fila_mensaje[imagen]' class='img-fluid desactivada'></td>";
                        }else{
                            echo "<td><img src='../../assets/imagenes/usuarios/perfil/$fila_mensaje[imagen]' class='img-fluid'></td>";
                        }
                        
                        echo "<td>
                            <form method='POST' action='ver_perfil_usuario.php'>
                                <input type='hidden' name='dni' value='$fila_mensaje[dni]'>
                                <input type='submit' name='enviarDni' value='Ver perfil' class='btn boton_analogo'>
                            </form>
                         </td>
                         ";
            ...
            echo "<h1 class='fw-bold text-center'>No hay usuarios aún</h1>";
        }
    ...
    <?php
        if(isset(($_POST["borrar"]))){
            ...
            $consulta_borrar="UPDATE usuario SET activo='0' WHERE dni=?";
            $resultado_borrar=$conexion->prepare($consulta_borrar);
            $resultado_borrar->bind_param("s", $dni);
            ...
            $consulta_borrar="UPDATE usuario SET activo='1' WHERE dni=?";
            $resultado_borrar=$conexion->prepare($consulta_borrar);
            $resultado_borrar->bind_param("s", $dni);
            $resultado_borrar->execute();
            $resultado_borrar->close();
