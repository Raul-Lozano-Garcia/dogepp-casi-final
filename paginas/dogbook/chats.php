...
<body>
    ...
    <main id="swup" class="transition-fade container">
        ...
        <section id="chats">
            ...
            <?php
                //SACO LOS CHATS
                $consulta_mensaje="SELECT dni,imagen,amigo.nombre nombre FROM mensaje INNER JOIN usuario ON dni=id_usuario_envia INNER JOIN amigo ON dni=id_usuario_amigo WHERE id_usuario='$_SESSION[dni]' GROUP BY dni ORDER BY mensaje.fecha DESC, hora DESC";
                ...
                if($filas_devueltas===0){
                    echo "<div class='aun_no text-center'><h1>Aún no tienes chats. Para agregar amigos y poder conversar con ellos dirígete a Dogbook > Amigos</h1></div>";
                }else{
                    while ($fila_mensaje=$datos_mensaje->fetch_array(MYSQLI_ASSOC)){

                        // AQUÍ SACO EL ÚLTIMO MENSAJE DE TODA LA CONVERSACIÓN ENTRE ESTE USUARIO Y YO
